...
global  cg_itoa:function
global  cg_utoa:function
global  cg_write_int:function
...
;;   cg_itoa
;; SYNOPSIS
;;   mov     rdi, <signed integer>
;;   mov/lea rsi, <buffer address>
;;   call    cg_itoa
;;
;;   extern struct utoa_result cg_itoa(int64_t n, char *buf);
;; FUNCTION
;;   Converts a signed integer to an ASCII string.
;; INPUTS
...
;;   * R9 - Clobbered
;;   * R10 - Clobbered
;;   * R11 - Clobbered (Used for sign tracking)
;; SOURCE
align 64
cg_itoa:
        mov     r11, rdi                ; Save original number to check sign later
        mov     rax, rdi                ; RAX = Dividend
        test    rax, rax
        jns     .is_positive            ; If positive, jump over the negation
        neg     rax                     ; 2's complement negation (handles INT64_MIN perfectly)
.is_positive:
        lea     rcx, [rsi + 20]         ; RCX = End of buffer (21 bytes total)
...
.loop:
        ...
        mulx    r8, rax, rax    ; BMI2 Fast Multiply: High bits in R8, low in RAX
        ...
        jnz     short .loop
        ;-----------------------------------------------------------------------
        test    r11, r11        ; Check the original number we saved
        jns     .done           ; If it was positive, we are done
        dec     rcx
        mov     byte [rcx], '-' ; Prepend the minus sign
.done:
...
cg_write_uint:
        ...
        mov     rbp, rsp
        push    rsi             ; Save file descriptor. It lives safely at [rbp - 8]
        sub     rsp, 32         ; Allocate 32 bytes for the buffer BELOW the saved fd
        ;-----------------------------------------------------------------------
...
;; FUNCTION
;;   Converts a signed integer to an ASCII string and writes it to a file
;;   descriptor.
;; INPUTS
;;   * RDI - Signed 64-bit integer
;;   * RSI - File descriptor
...
cg_write_int:
        push    rbp             ; Save stack frame
        mov     rbp, rsp
        ; --- THE SAFE STACK LAYOUT ---
        push    rsi             ; Save file descriptor. It lives safely at [rbp - 8]
        sub     rsp, 32         ; Allocate 32 bytes for the buffer BELOW the saved fd
        ;-----------------------------------------------------------------------
        mov     rsi, rsp        ; RSI = Pointer to the safe stack buffer
        ; RDI is already the integer to convert
        call    cg_itoa         ; RAX = Pointer to string start
                                ; RDX = Length of string
        ;-----------------------------------------------------------------------
        mov     rdi, [rbp - 8]  ; Retrieve the unmodified file descriptor from the stack
        mov     rsi, rax        ; RSI = Pointer to string start
        ; RDX is already the length of the string from cg_itoa
        mov     rax, 1          ; SYS_write (1 on x86_64)
        syscall
        ;-----------------------------------------------------------------------
        leave                   ; Automatically destroys the 32-byte buffer and restores RBP/RSP
        ret
;;******
